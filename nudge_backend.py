...
@app.route('/')
def admin_page():
    return send_from_directory('.', 'nudge_admin.html')

@app.route('/me/<token>')
def client_page(token):
    return send_from_directory('.', 'nudge_client.html')
...
def view_invoice(token):
    db = sqlite3.connect(DB_PATH); db.row_factory = sqlite3.Row
    i = db.execute("""SELECT i.*,c.name,s.date,s.time,s.service,b.name as bn,b.phone as bp
    ...
    <link href="https://fonts.googleapis.com/css2?family=Great+Vibes&family=Libre+Franklin:wght@400;600;700&family=IBM+Plex+Mono:wght@400;600&display=swap" rel="stylesheet">
    <style>*{{margin:0;padding:0;box-sizing:border-box}}body{{font-family:'Libre Franklin',sans-serif;max-width:560px;margin:40px auto;padding:20px;color:#2a1a0a;background:#faf5eb}}
    h1{{font-family:'Great Vibes',cursive;color:#cc0000;font-size:2rem;margin-bottom:4px}}
    .sub{{font-size:.7rem;color:#8a7a6a;margin-bottom:24px}}
    table{{width:100%;border-collapse:collapse}}td{{padding:10px;border-bottom:1px solid #e8dcc8;font-size:.8rem}}
    td:last-child{{text-align:right;font-family:'IBM Plex Mono';font-weight:600}}
    .total td{{font-weight:700;font-size:1rem;color:#cc0000;border-bottom:none}}
    .status{{display:inline-block;padding:4px 12px;border-radius:12px;font-family:'IBM Plex Mono';font-size:.55rem;font-weight:600;letter-spacing:1px;margin-top:16px}}
    .unpaid{{background:rgba(204,0,0,.06);color:#cc0000}}.paid{{background:#e6f5ea;color:#1a7a2e}}
    .foot{{font-size:.6rem;color:#8a7a6a;margin-top:24px;text-align:center}}</style></head>
    <body><h1>{i['bn']}</h1><div class="sub">Invoice for {i['name']}</div>
    <table><tr><td>{i['service']}</td><td>{i['date']}</td><td>{i['time']}</td><td>${i['amount']:.2f}</td></tr>
    <tr class="total"><td colspan="3">Total</td><td>${i['amount']:.2f}</td></tr></table>
    <div class="status {'paid' if i['status']=='paid' else 'unpaid'}">{i['status'].upper()}</div>
    <div class="foot">{i['bn']} · {i['bp']}<br>Powered by Nudge</div></body></html>"""
...
def a_client_detail(cid):
    ...
    return jsonify({'client': dict(client), 'visits': [dict(v) for v in visits]})
...
def c_nudge(token):
    d = request.json; db = get_db(); biz = g.biz
    fs = db.execute("SELECT * FROM slots WHERE id=? AND client_id=?", (d['from_slot_id'], g.client_id)).fetchone()
    ...
    ts = db.execute("SELECT s.*,c.name as cn,c.phone as cp,c.id as cid FROM slots s JOIN clients c ON s.client_id=c.id WHERE s.id=? AND s.biz_id=?",
        (d['to_slot_id'], g.biz_id)).fetchone()
    if not ts: return jsonify({'error':'Slot not available'}), 400
    mx = biz.get('max_offers', 2)
    att = get_attempts(db, g.biz_id, g.client_id, d['to_slot_id'])
    if att >= mx: return jsonify({'error':f'Max {mx} offers reached'}), 400
    attempt = inc_attempts(db, g.biz_id, g.client_id, d['to_slot_id'])
    exp = (datetime.now() + timedelta(minutes=biz.get('offer_timer_min', 30))).isoformat()
    fee = d.get('fee', 0)
    db.execute("INSERT INTO nudges (biz_id,from_slot_id,to_slot_id,from_client_id,to_client_id,attempt,fee,expires_at) VALUES (?,?,?,?,?,?,?,?)",
        (g.biz_id, d['from_slot_id'], d['to_slot_id'], g.client_id, ts['cid'], attempt, fee, exp))
    nid = db.execute("SELECT last_insert_rowid()").fetchone()[0]; db.commit()
    fin = " This is their final offer." if attempt >= mx else ""
...
def c_nudges(token):
    db = get_db()
    inc = db.execute("""SELECT n.*,fc.name as from_name,fs.time as from_time,ts.time as to_time
        FROM nudges n JOIN clients fc ON n.from_client_id=fc.id JOIN slots fs ON n.from_slot_id=fs.id JOIN slots ts ON n.to_slot_id=ts.id
        WHERE n.to_client_id=? AND n.status='pending' ORDER BY n.created_at DESC""", (g.client_id,)).fetchall()
    out = db.execute("""SELECT n.*,tc.name as to_name,fs.time as from_time,ts.time as to_time
        FROM nudges n JOIN clients tc ON n.to_client_id=tc.id JOIN slots fs ON n.from_slot_id=fs.id JOIN slots ts ON n.to_slot_id=ts.id
